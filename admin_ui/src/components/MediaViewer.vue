<template>
  <div id="media_viewer" v-if="mediaViewerConfig">
    ...
    <div class="top_bar">
      <p class="file_key"><span>File:</span> {{ mediaViewerConfig.fileKey }}</p>
    ...
    <template v-if="fileURL">
      <div v-if="isImage(mediaViewerConfig.fileKey)" class="image_container">
        <img :src="fileURL" />
      </div>
      ...
      <div v-else-if="isPDF(mediaViewerConfig.fileKey)" class="pdf_container">
        <iframe :src="fileURL"></iframe>
      </div>

      <div class="no_preview" v-else>
        <p>
          <font-awesome-icon icon="file"></font-awesome-icon> Unable to preview
          file - download below.
        </p>
      </div>
    </template>
...
export default defineComponent({
  ...
  methods: {
    ...
    async generateFileURL({
      ...
      const response = await axios.post(`${BASE_URL}media/generate-file-url/`, {
