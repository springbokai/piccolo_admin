<template>
  <div>
    ...
    <template v-else-if="type == 'string'">
      <template
        v-if="format && ['date-time', 'date', 'time'].indexOf(format) != -1"
      >
        <OperatorField :columnName="columnName" v-if="isFilter" />
        ...
        <template v-if="format == 'time'">
          ...
          />
          <input type="hidden" v-bind:name="columnName" v-model="localValue" />
        </template>
    ...
    <template v-else-if="type == 'boolean'">
      <select v-bind:name="columnName">
        ...
        <option v-bind:selected="value == true" value="true">True</option>
        <option v-bind:selected="value == false" value="false">False</option>
      </select>
    </template>
  ...
  methods: {
    ...
    async uploadFile(event: Event) {
      ...
      try {
        ...
        if (this.type == "array") {
          if (this.localValue) {
            this.localValue = [...this.localValue, response.data.file_key];
          } else {
