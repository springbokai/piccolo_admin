<template>
  <BaseView>
    <template v-if="!loading">
        ...
        <div class="title_bar">
          ...
          <div class="buttons">
            <a
              ...
              v-on:click.prevent="showUpdateModal = !showUpdateModal"
            >
              ...
              <template v-else>
                <table>
                  <tr>
                    ...
                    <th v-bind:key="name" v-for="name in visibleColumnNames">
                      {{
                      ...
                      <a
                        href="#"
                        @click.prevent="showSortModal = true"
                        v-if="orderByMapping[name]"
                      >
                        <font-awesome-icon
                          icon="caret-up"
                          v-if="orderByMapping[name].ascending"
                        />
                  ...
                  <tr v-bind:key="row[pkName]" v-for="row in rows">
                    ...
                    <td v-bind:key="name" v-for="name in visibleColumnNames">
                      <span v-if="row[name] === null">
                        <code>NULL</code>
                      </span>
                      <span class="link" v-else-if="name == linkColumnName">
                        <router-link
                          :to="{
                            name: 'editRow',
                            params: {
                      ...
                      <span v-else-if="choicesLookup[name]">
                        <template v-if="isArray(name)">
                          {{
                            abbreviate(
                              row[name]
                                .map((i: any) => choicesLookup[name]![i] ?? i)
                                .join(", ")
                            )
                          }}
                        </template>
                        <template v-else>{{
                          choicesLookup[name]![row[name]] ?? row[name]
                        }}</template>
                      </span>
                      <span
                        class="link"
                      ...
                      <span v-else-if="isMediaColumn(name)">
                        <template v-if="isArray(name)">
                          <a
                            style="display: block"
                            href="#"
                            v-for="item in row[name]"
                            :key="item"
                            @click.prevent="showMedia(item, name)"
                            >{{ abbreviate(item) }}
                          </a>
                        </template>
                        <template v-else>
                          <a
                            href="#"
                            @click.prevent="showMedia(row[name], name)"
                            >{{ abbreviate(row[name]) }}
                          </a>
                        </template>
                      </span>
      ...
      <div class="right_column" v-if="showFilter" data-uitest="right_sidebar">
        <RowFilter
...
export default defineComponent({
  ...
  methods: {
    ...
    selectAllRows() {
      ...
      if (this.allSelected) {
        this.selectedRows = this.rows.map((row: any) => row[this.pkName]);
      } else {
        this.selectedRows = [];
      }
    },
    ...
    async fetchSchema() {
      ...
      if (orderBy) {
        this.$store.commit("updateOrderBy", deserialiseOrderByString(orderBy));
      } else {
