...
export default {
  ...
  actions: {
    async setupTranslations(context: Context) {
      ...
      const availableLanguageCodes = data.translations.map((translation) => {
      ...
      const storedLanguagePreference = localStorageUtils.getDefaultLanguage();
      ...
      if (
        ...
        if (data.default_language_code == "auto") {
          ...
          if (availableLanguageCodes.indexOf(browserLanguage) != -1) {
            ...
            if (availableLanguageCodes.indexOf(_browserLanguage) != -1) {
